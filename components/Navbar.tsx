  ...
  return (
    <>
      <header 
        className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-300 ${
          scrolled 
            ? (theme === 'dark' ? 'bg-black/95 border-b border-white/10 py-3' : 'bg-white/95 border-b border-slate-200 py-3 shadow-md') 
            : 'bg-transparent py-5'
        }`} 
        style={{ backdropFilter: 'blur(12px)' }}
      >
      ...
      {/* MOBILE MENU */}
      {isOpen && (
        <div className={`fixed inset-0 z-[90] lg:hidden flex flex-col justify-center px-8 gap-8 animate-in slide-in-from-bottom duration-300 ${
          theme === 'dark' ? 'bg-black text-white' : 'bg-white text-slate-900'
        }`}>
